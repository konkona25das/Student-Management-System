...
class resultClass:
    def __init__(self, root):
        ...
        #====title====
        title= Label(self.root, text= "Add student results", font=("goudy old style",20, "bold"), bg= "orange",fg="#262626").place(x=10, y=15,width=1275, height= 65)
    #=======widgets=======
    #=======variable=======
        ...
        txt_serach_name= Entry(self.root,textvariable=self.var_name, font=("goudy old style", 20, "bold"),bg="light yellow", state= 'readonly').place(x= 280, y=160,width= 300)
        txt_serach_course= Entry(self.root,textvariable=self.var_course, font=("goudy old style", 20, "bold"),bg="light yellow", state= 'readonly').place(x= 280, y=220,width= 300)
        txt_serach_marks_ob= Entry(self.root,textvariable=self.var_marks, font=("goudy old style", 20, "bold"),bg="light yellow").place(x= 280, y=280,width= 300)
        txt_serach_full_marks= Entry(self.root,textvariable=self.var_full_marks, font=("goudy old style", 20, "bold"),bg="light yellow").place(x= 280, y=340,width= 300)
        ...
        btn_clear= Button(self.root, text="Clear", font=("goudy old style", 15), bg= "lightgray", activebackground= "lightgray", cursor="hand2", command= self.clear).place(x=420,y= 420, width= 120, height=35)
 
        #=====image=====
        self.bg_img= Image.open("images/result.jpg")
        self.bg_img= self.bg_img.resize((600,300), Image.Resampling.LANCZOS)
