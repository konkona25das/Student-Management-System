...
class StudentClass:
    def __init__(self, root):
        ...
        #====title====
        title= Label(self.root, text= "Manage Student Details", font=("goudy old style",20, "bold"), bg= "#033054",fg="white").place(x=10, y=15,width=1275, height= 40)
        ...
        lbl_address= Label(self.root, text= "Address", font=("goudy old style", 15, "bold"),bg="white").place(x= 15, y=250)

        #=====Entry Fields=====
        self.txt_roll= Entry(self.root,textvariable=self.var_rollno, font=("goudy old style", 15, "bold"),bg="light yellow")
        ...
        self.txt_address.place(x= 150, y=267, width= 540, height= 140)
        ...
        lbl_search_rolle= Label(self.root, text= "Roll No", font=("goudy old style", 15, "bold"),bg="white").place(x= 720, y=60)
        txt_serach_roll= Entry(self.root,textvariable=self.var_search, font=("goudy old style", 15, "bold"),bg="light yellow").place(x= 880, y=60,width= 220)
        btn_search= Button(self.root, text="Search", font=("goudy old style", 15, "bold"), bg= "#03a9f4", fg= "white", cursor="hand2", command= self.search).place(x=1140,y= 60, width= 120, height=28)
    
        #=====Content=====
        self.C_Frame= Frame(self.root, bd= 2, relief= RIDGE)
        self.C_Frame.place(x=700,y= 100, width= 560, height=340)
        ...
        self.CourseTable.column("contact", width=50)
        self.CourseTable.column("admission", width=50)
        self.CourseTable.column("state", width=50)
        self.CourseTable.column("city", width=50)
        self.CourseTable.column("pin", width=50)
        self.CourseTable.column("course", width=50)
        ...
        try:
            if self.var_rollno.get()== "":
                ...
                if row== None:
                    ...
                    if op== True:
                        cur.execute("delete from student where roll=?,"(self.var.get(),))
                        con.commit()
        ...
        try:
            if self.var_rollno.get()== "":
                ...
                if row== None:
                    ...
                    cur.execute("UPDATE student name=?, email=?, gender=?, dob=?, contact=?, admission=?,course=?, state=?, city=?,pin=?, address=? where roll=?",
                                (
